...
def init_env():
    ...
    source_envfile('/home/cakgod/hdl_graph_slam/devel/setup.bash')
...
lidar_proc = subprocess.Popen(shlex.split('roslaunch hdl_graph_slam hdl_graph_slam.launch'), env=os.environ.copy(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
def run_lidar_cmd():
...
top_proc = Process(target=parse_top_result, args=(get_pid('velodyne_nodelet_manager'),))
top_proc.start()
...
'''
playbag_proc = Process(target=run_cmd, args=('rosbag play --clock /mnt/shared/dataset/KITTI/10.bag', os.environ.copy()))
playbag_proc.start()
playbag_proc.join()
...
with open('/home/cakgod/temp/KITTI_cpu_memory_pc.txt', 'w') as f:
